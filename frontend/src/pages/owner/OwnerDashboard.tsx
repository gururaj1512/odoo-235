...
const OwnerDashboard: React.FC = () => {
  ...
  useEffect(() => {
    if (user?.role === 'Owner') {
      ...
      dispatch(fetchOwnerBookingTrends(selectedPeriod));
      dispatch(fetchOwnerPeakHours());
    }
  }, [dispatch, user, selectedPeriod]);

  // Use dynamic stats if available, otherwise fallback to calculated stats
  const totalBookings = ownerStats?.totalBookings || bookings.length;
  const activeCourts = ownerStats?.activeCourts || facilities.reduce((acc, facility) => acc + (facility.courts?.length || 0), 0);
  const totalEarnings = ownerStats?.totalEarnings || bookings.reduce((acc, booking) => acc + booking.totalAmount, 0);
  const pendingBookings = ownerStats?.pendingBookings || bookings.filter(booking => booking.status === 'Pending').length;
  ...
  const formatPeakHours = () => {
    ...
    return peakHours.slice(0, 9); // Show top 9 hours
  };
  ...
  if (facilitiesLoading || bookingsLoading || ownerLoading) return <LoadingSpinner />;
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      <div className="bg-white rounded-2xl shadow-sm border p-6">
                        ...
                        <div className="h-64 flex items-end justify-between gap-2">
                          {formatBookingTrends().map((day, index) => (
                            <div key={day.day} className="flex-1 flex flex-col items-center">
                              <div className="w-full bg-gray-200 rounded-t-lg relative">
                                <div
                                  ...
                                  style={{ 
                                    height: `${day.bookings > 0 ? (day.bookings / Math.max(...formatBookingTrends().map(t => t.bookings))) * 100 : 0}%`,
                            ...
                            <div key={hour.hour} className="flex items-center gap-3">
                              ...
                              <div className="flex-1 bg-gray-200 rounded-full h-2">
                                <div
                                  ...
                                  style={{ width: `${hour.bookings > 0 ? (hour.bookings / Math.max(...formatPeakHours().map(h => h.bookings))) * 100 : 0}%` }}
                                />
                              </div>
